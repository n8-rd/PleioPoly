//a SLiM model of the joint evolution of quantitative habitat-specific performance and preference. In this version pleiotropies affect host us performance
...
initialize() {
	...
	prefRat = (pp + 1) * pr;
	defineConstant("Mutation_mix", c(pp,1,1,prefRat)); //proportion of Pleiotropic, and CN traits
	defineConstant("Mutation_var",c(mv));
	defineConstant("delto", delo); //rate at which performance optima change
	defineConstant("pK1", pk1);
	defineConstant("K1", asInteger(pK1*1000)); // host 1 carrying capacity
	defineConstant("K2", asInteger(1000-K1)); //host 2 carrying capacity
	// QTL-related constants used below
    defineConstant("QTL_mu", c(0, 0)); // pleio effect means
    defineConstant("QTL_cov", cv); // |max out cov |started with 0.25 pleio effect covariance
    defineConstant("QTL_sigma", matrix(c(Mutation_var,QTL_cov,QTL_cov,Mutation_var), nrow=2));
	...
	initializeMutationRate(1e-7); // dropping this, maybe we just have too much diversity.
	initializeMutationType("m1", 0.5, "f", 0.0);   // neutral mutations
	initializeMutationType("m2", 0.5, "n", 0.0, 0.0);   // Pleiotropic performance QTLs
	...
	initializeRecombinationRate(1e-4); //8
}
...
//Map performance phenotypes to viability.
//Starting after generations of burn-in
1000: early() {
	...
	for (ind in inds){
	    ...
	    scale = dnorm(p1_new_opt, p1_new_opt, Selection_strength[0]); //quantiles, mean, sd
		effect0 = dnorm(p1_new_opt, phenotype1, Selection_strength[0]) / scale;
		effect1 = dnorm(p2_new_opt, phenotype2, Selection_strength[0]) / scale;
...
//start outputs after the burnin
//stop sim when the both mean phenos are within a threshold of the target
1000:2000 late() {
    inds = sim.subpopulations.individuals;
    if (!size(inds)){
	    ...
	    mPrefP1 = size(p1.individuals) ? mean(p1.individuals.sumOfMutationsOfType(m5)) else 0.0;
	    mPrefP2 = size(p2.individuals) ? mean(p2.individuals.sumOfMutationsOfType(m5)) else 0.0;
	    pref_p1 = size(mPrefP1) ? 1/(1+exp(-1*mPrefP1)) else "NA";
	    pref_p2 = size(mPrefP2) ? 1/(1+exp(-1*mPrefP2)) else "NA";
        ...
        Shan = Hp; 
        fst = 0.0;
        if (size(p1.individuals) & size(p2.individuals)){
            fst = calcFST(p1.genomes, p2.genomes);
        }
        writeFile("Highway.csv", paste(c(mp1P1,mp2P2,mPrefP1,mPrefP2,Np1,Np2,bWd,(sim.generation-1),ss,mv,pp,pr,cv,H,Shan,delto,fst,mW,pK1,RunId),sep=','), append=T);
    }
}
